...
func (p *parser) Parse(tag string) (map[string]Definition, error) {
	...
	for {
		...
		case scanner.Ident:
			...
			if p.s.Peek() == '=' {
				...
				result[ident] = newDefinition(ident, arg)
			} else if p.s.Peek() == '(' {
				p.s.Next()
				arg, err := p.parseArgs()
				if err != nil {
				...
				result[ident] = newDefinition(ident, map[string]interface{}{})
			}
		case ',':
			// NOP
		default:
...
func ParseTag(value string, name string) (map[string]Definition, error) {
	p := newParser(name)
	return p.Parse(value)
}

// ParseStruct parses struct tags of given object. map key is a field name.
func ParseStruct(obj interface{}, tag string) (map[string]map[string]Definition, error) {

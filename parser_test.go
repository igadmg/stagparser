...
func TestExampleSuccess(t *testing.T) {
	...
	if v, ok := abc.Attributes["abc"]; !ok || v.(int64) != 1 {
	...
	if v, ok := def.Attributes["def"]; !ok || v.(string) != "ghi" {
	...
	if v, ok := jkl.Attributes["jkl"]; !ok || v.(string) != "mno" {
	...
	if v, ok := stu.Attributes["vwx"]; !ok || v.(string) != "ccc" {
		t.Fatalf("vwx attribute should be \"ccc\" but got %v(%T)", v, v)
	}
	if v, ok := stu.Attributes["zzz"]; !ok || v.(string) != "ddd" {
	...
	if v, ok := abd.Attributes["abd"]; !ok || v.(string) != "\r\n'" {
